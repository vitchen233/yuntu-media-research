...
6. 单条确实需要完整文案时，按`references/local-transcription.md`检测并调用Skill外的FunASR；不要预装或捆绑它。
7. 没有浏览器时仍可完成RedFox批量分析，但报告必须标记“单条页面未人工核对”。
8. 只有RedFox密钥和浏览器都不可用时，才向用户说明缺少哪项能力；不要要求用户代为整理整批数据。
